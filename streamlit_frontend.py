...
if not threads:
   ...
   for thread_id in threads:
    ...
    title = get_chat_title(str(thread_id))
    ...
    with col2:
     if st.button(
        ...
        st.session_state["chat_threads"] = [
            ...
            if str(t) != str(thread_id)
        ]

        if str(thread_id) == str(st.session_state["thread_id"]):
            reset_chat()
...
if user_input:
    ...
    if get_chat_title(thread_key) is None:
        title = generate_chat_title(user_input)
        save_chat_title(thread_key, title)
